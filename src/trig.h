// Fixed-point trig. Tier 1: a table, because the sim may not call libm and
// must produce bit-identical results on MinGW and wasm.
//
// Generated, do not hand-edit. 360 entries of sin() in Q16.16, indexed by
// whole degrees; the leg swings at ~6-40 deg/frame so 1-degree resolution is
// far finer than anything visible, and quantising the index keeps the lookup
...
static const fx OB_SIN_DEG[360] = {
...
// Angle in Q16.16 DEGREES. Any magnitude is fine; the index wraps.
static inline fx fx_sin_deg(fx deg) {
  int32_t d = FX_FLOOR(deg) % 360;
  if (d < 0) d += 360;
  return OB_SIN_DEG[d];
}
static inline fx fx_cos_deg(fx deg) { return fx_sin_deg(deg + FXI(90)); }
